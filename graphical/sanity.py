...
def check_parameter_recovery(name, recovered_mean, recovered_sigma, truth, k=3.0):
    ...
    if recovered_sigma is None or recovered_sigma == 0.0:
        ...
        msg = (f"{name}: mean={recovered_mean:.4g} truth={truth:.4g} "
               f"sigma=0 (deterministic) -> {'PASS' if passed else 'FAIL'}")
        return passed, msg
    z = abs(recovered_mean - truth) / recovered_sigma
    passed = z <= k
    msg = (f"{name}: mean={recovered_mean:.4g} ± {recovered_sigma:.4g} "
           f"truth={truth:.4g} |z|={z:.2f} (k={k}) -> "
           f"{'PASS' if passed else 'FAIL'}")
    return passed, msg


def check_max_log_likelihood(max_ll, truth_ll_sum, n_datasets,
                             tolerance_per_dataset=5.0):
    ...
    msg = (f"max_log_likelihood: {max_ll:.3f} vs truth_sum={truth_ll_sum:.3f} "
...
def print_sanity_summary(results, header="Sanity checks"):
    ...
    print(f"SANITY: {'PASS' if aggregate else 'FAIL'} "
